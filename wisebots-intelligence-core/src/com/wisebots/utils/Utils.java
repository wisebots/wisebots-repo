...
public final class Utils {
	...
	public static void printStatistics(GameCache gcache,
			long player1, long player2, long draw, long match, long totalmatch,
			String strategy) {
		/*
		 * if(match%1000==0 && match>0){ DecimalFormat df = new
		 * DecimalFormat("0.00"); try { FileWriter fw = new FileWriter(path +
		 * strategy + ".txt", true); fw.write(player1 + ";" +
		 * df.format((double)player1*100/match) + ";" + player2 + ";" +
		 * df.format((double)player2*100/match) + ";" + draw + ";" +
		 * df.format((double)draw*100/match) + ";" + match + ";" + totalmatch +
	...
	public static int[] getStateArrayFromArray(int[] array) {
	...
	*/
	...
	public static int[] getArrayFromString(String sarray){
		String strNumbers = sarray.replaceAll(", ", "").replace("[", "").replace("]", "");
	
		int[] narray = new int[strNumbers.length()];
		for(int i=0; i< strNumbers.length(); i++){
			narray[i] = Integer.parseInt(""+strNumbers.charAt(i));
		}
	...
	public static void saveQValues(HashMap<String, Double> qvalue, String name) {
		/*
		 ...
		 System.out.println("Tamanho: " +qvalue.size()); 
		 try { 
			 FileWriter fw = new FileWriter(path + "qvalues-" + name + ".txt"); 
			 while(iterator.hasNext()) { 
				 String key = iterator.next(); 
			 	long action = key%100; long state = (key-action)/100; 
			 	Double value = qvalue.get(key) ; 
			 	StringBuffer sb = new StringBuffer();
			 	sb.append("(").append(state).append(",").append(action).append(")").append("=").append(value).append("\n"); fw.write(sb.toString()); }
			 	fw.close(); 
			 } 
		 catch (IOException e) 
		 { 
	...
	public static void putBase(Cache cache, String key, double value) {
		try {
			cache.put(key, value);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static double getBase(Cache cache, String key) {
		try {
	...
	public static void printCacheQValues(HashMap<String, Object> qvalue) {
		...
		System.out.println("PRINTINGGGGGG Q-VALUES");
		while (iterator.hasNext()) {
			String key = iterator.next();
...
*/
	...
	public static void main(String[] args) {
		...
		String strNumbers = a.replaceAll(", ", "").replace("[", "").replace("]", "");
		...
		String b = Arrays.toString(narray);
